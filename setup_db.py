"""
Database Setup Script for FNCS Categories
==========================================
This script sets up the Supabase PostgreSQL database by:
1. Testing the connection
2. Creating the categories table
...
def execute_sql_file(database_url, sql_file_path):
    ...
    print_header("STEP 3: Creating Categories Table")
    ...
    try:
        ...
        print_success("Categories table created")
...
def verify_setup(database_url):
    ...
    try:
        ...
        if not table_exists:
            print_error("Categories table does not exist!")
            cursor.close()
            conn.close()
            return False

        print_success("Categories table exists")
        ...
        print_success(f"Total categories: {count}")
        ...
        print_info("Categories list:")
        print("\n" + "-" * 80)
        print(f"{'ID':<5} {'Name':<30} {'Active':<10} {'Created At':<25}")
        print("-" * 80)

        for cat in categories:
            cat_id, name, is_active, created_at = cat
            status = "✓ Active" if is_active else "✗ Inactive"
            created_str = created_at.strftime("%Y-%m-%d %H:%M:%S") if created_at else "N/A"
            print(f"{cat_id:<5} {name:<30} {status:<10} {created_str:<25}")
...
def main():
    """Main setup function"""
    print_header("FNCS Database Setup - Starting")
    ...
    if not execute_sql_file(database_url, 'create_table.sql'):
        print_error("Setup failed: Could not create tables")
        return False
    ...
    print("2. Test the API: python consumir.py")
    print("3. Access API documentation in README.md")
